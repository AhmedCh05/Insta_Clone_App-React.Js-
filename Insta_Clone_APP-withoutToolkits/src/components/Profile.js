...
const Profile = ({ data }) => {
  ...
  return (
    <div className = "App">
      ...
      <div style={{paddingTop:"5%",border:"2px solid black"}} className="d-flex widt flex-wrap mt-4">
        {filtered[0].postImages.map((item) => {
          return <img style = {{padding:"4px",height:"55%",width:"45%"}} src={item.postImage} alt="Post" />;
        })}
      </div>
    </div>
  );
